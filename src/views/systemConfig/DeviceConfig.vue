<template>
  <div class="device_config">
    ...
    <el-card class="box-card">
      ...
      <el-row type="flex" class="row-bg deviceInfo">
        <el-col class="topscroll" :span="16">
          <em class="table-title">相位与信号灯关联</em>
          <!-- 上面类表 -->
          <div class="grid">
            <gemini-scrollbar style="height:290px">
              ...
              >
                <el-table-column label="相位序号" align="center">
                ...
                <el-table-column label="相位类型" align="center">
                  <template slot-scope="scope">
                    <span>{{
                      scope.row.type === 1 ? "机动车相位" : "行人相位"
                    }}</span>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="状态" align="center">
                  <template slot-scope="scope">
                    <span
                      :style="
                        scope.row.light_info.light_num === '未关联'
                          ? 'color: #ccc'
                          : ''
                      "
                      >{{
                        scope.row.light_info.light_num === "未关联"
                          ? "未关联"
                          : scope.row.light_info.light_num
                      }}</span
                    >
                  </template>
                </el-table-column>
        ...
        <!-- 右侧图示 -->
        <el-col :span="6" style="minwidth:400px">
          <div class="diagram">
            <div
      ...
      >
        ...
        >
          ...
          <el-form-item label="编号:" prop="camera_num">
            <el-input
              class="infoInputs"
          ...
          <el-form-item label="架设高度:" class="height" prop="height">
            <el-input
              class="infoInputs"
              v-model="formdata.height"
              maxlength="3"
              placeholder="请输入摄像头高度"
            ></el-input
            >&nbsp;m
          </el-form-item>
          ...
          <el-form-item label="后缀:" prop="tail">
            <el-input
              class="infoInputs"
      ...
      <el-row>
        <el-col :span="24">
          <em class="table-title">摄像头配置</em>
          <div class="grid">
            <gemini-scrollbar style="height:313px">
              <el-table
                :data="cameraData"
                :header-cell-style="{ background: '#FAFAFA' }"
                class="device-info"
              >
                <el-table-column label="方向" align="center">
                  <template slot-scope="scope">
                    <span>{{
                      scope.row.direct_type === ""
                        ? "---"
                        : scope.row.direct_type
                    }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="道路类型" width="100" align="center">
                  <template slot-scope="scope">
                    {{
                ...
                <el-table-column label="架设高度（米）" align="center">
                  <template slot-scope="scope">
                    <span>{{
                      scope.row.height ? scope.row.height + "m" : "---"
                    }}</span>
                  </template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column label="摄像头编号" align="center">
                  <template slot-scope="scope">
                    <span>{{
                      scope.row.camera_num === "未关联"
                        ? "---"
                        : scope.row.camera_num
                    }}</span>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="操作" class="font_org" align="center">
                  <template slot-scope="scope">
                    <el-button
                      type="text"
                      size="mini"
                      color="#FF6600"
                      @click="handleEdit(scope.$index, scope.row, 'formdata')"
                      ><i style="color:#FF6600; font-style:normal"
                        >设置</i
                      ></el-button
                    >
                    <el-button
                      type="text"
                      size="mini"
                      @click="handldel(scope.$index, scope.row)"
                      >重置</el-button
                    >
                  </template>
                </el-table-column>
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        // ],
        camera_num: [{ required: true, validator: cameraNum, trigger: 'blur' }],
        model: [{ required: true, validator: deviceModel, trigger: 'blur' }],
        height: [{ required: true, validator: height, trigger: 'blur' }],
        port: [{ validator: port, trigger: 'blur' }],
        ip: [{ required: true, validator: ip, trigger: 'blur' }],
        tail: [{ required: true, validator: tail, trigger: 'blur' }],
    ...
    next () {
      for (let i = 0; i < this.cameraArr.length; i++) {
        if (this.cameraArr[i] === '未关联') {
          this.$confirm('有摄像头未设置,无法观看实时视频, 是否继续?', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            center: true
          }).then(() => {
...
.hero .gm-scrollbar .thumb:hover, .hero .gm-scrollbar .thumb:active
...
.grid .gm-scrollbar .thumb
  background-color #D8D8D8
.grid .gm-scrollbar .thumb:hover, .grid .gm-scrollbar .thumb:active
  background-color #D8D8D8
.gm-scrollbar .thumb
  border-radius 4px
